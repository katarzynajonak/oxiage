...
import dash
from dbUtil import DBgetCellularComponentNamesAll
...
dash.register_page(__name__,
                   path='/search_results_tab_Annotations',
                   title='Search Results Annotations',
...
def layout(lang_ID=None, UniGene=None, organism_ID=None):
    ...
    if UniGene != None and organism_ID != None:
        if organism_ID == '1' or organism_ID == '2' or organism_ID == '3' or organism_ID == '4':
            ...
            uniProtNamesFromDB = DBgetCellularComponentNamesAll(mydb, organism_ID, UniGene)
            ...
            dbc.Row([
                ...
                dbc.Col([
                    html.Div(style=CONTENT_STYLE, children=[
                        html.H1(children=[
                            html.Span('Results for Cellular Component Search ', style={'color': '#3d3d3d'}), ], ),
                        html.H4(
                            children=[html.Span('This table contains proteins localized in chosen cellular compartment ',
                                                style={'color': '#3d3d3d'}), ], ),
                        html.Hr(),
                        dmc.Breadcrumbs(
                            children=[
                                ...
                                dcc.Link([html.Span('Cellular Component Search')],
                                         href=f'/search_results_tab_Annotations?lang_ID={lang_ID}&UniGene={UniGene}&organism_ID={organism_ID}'
                                         ),
                            ],
                        ),
                        html.Br(),
                        ...
                        dbc.Row([dbc.Col([
                            html.H4(children=[html.Span('Proteins localized in: ', style={'color': '#3d3d3d'}),
                            ...
                            dbc.Button(children=[html.I(className="bi bi-download"),
                                       ...
                                       id='btn-download-annot'),
                            dcc.Download(id="download-component-annot"),
...
@callback(
    Output('download-component-annot', 'data'),
    Input('tbl-annot', 'data'),
    Input('btn-download-annot', 'n_clicks'),
    prevent_initial_call=True,
)
def update_search_button(data, n_clicks):
    dataFrame = pd.DataFrame(data)
    dataFrame2 = dataFrame.drop(columns='UniProt ID Link')
    return dcc.send_data_frame(dataFrame2.to_csv, "OxiAge_CellularComponentSearch_Table.txt", sep=';')